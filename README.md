...
```
By default, the ```BinPRE/src``` directory contains a binary file that implements the Modbus protocol, which we provide. Additionally, we have included installation methods for two other protocols (EthernetIP and DNS) in the Dockerfile. When you install using the Dockerfile, the implementations for all three of these protocols will be automatically installed in the ```~/BinPRE/src``` directory.
...
### (Optional) testing other protocol implementations


Similar to the Modbus implementation mentioned previously, you are welcome to test other protocol implementations if you want. 


#### For users who have configured the running environment with the Dockerfile.

If you have set up the running environment with the Dockerfile, you can easily test the implementations of the other two protocols (EthernetIP, denoted as eip, and DNS, denoted as dns) - the installation methods for which have already been included in the Dockerfile.

##### Step1: start server
```
cd ~/BinPRE/Artifact_Evaluation/BinPRE_scripts
./run_<ProtocolName>_server.sh
# it should be ./run_eip_server.sh or ./run_dns_server.sh
```
For testing other protocol implementations, please pay attention to the tips in the script when the server under testing requires user interaction.

Note that the two alternative protocol implementations built with Dockerfile do not require user interaction.

##### Step2: run BinPRE
```
cd ~/BinPRE/Artifact_Evaluation/BinPRE_scripts
./run_<ProtocolName>_client.sh
# it should be ./run_eip_client.sh or ./run_dns_client.sh
```
When BinPRE has sent 50 messages and started waiting for user interaction, <u>please enter the corresponding thread number in the terminal</u> to provide BinPRE with the correct thread number to complete the analysis.

Note that the thread number may vary between different protocol implementations. Please check the required thread number in the scripts ```./run_<ProtocolName>_client.sh``` and make sure that you have entered the correct thread numbers for analysis. **For example, to check the thread number of EthernetIP, you can**:
```
cd ~/BinPRE/Artifact_Evaluation/BinPRE_scripts
cat ./run_eip_client.sh
```

The content of ```./run_eip_client.sh``` is shown as follows:
```
cd ../../Analyzer

python3 fsend_split.py eip 0 0 oa xx little 0
#wait and enter threadId: 0
#Total Analyze Time:481.130747795105l
``` 
Thus, the corresponding thread number for this EthernetIP server is '0'. Please enter '0' when BinPRE has sent 50 messages and started waiting for user interaction.

##### Step3: check results

Please refer to the introduction above for information on storing and viewing results.

#### For users who manually set up the running environment step by step

If you have manually set up the running environment step-by-step, please install the other protocol implementations you want to test in the ```~/BinPRE/src``` directory and remember to add the corresponding PCAP files and Groundtruth files in the ```BinPRE/Analyzer/Groundtruth/``` and ```BinPRE/Analyzer/Groundtruth/``` directories, respectively. 

Similar to the steps provided for users who set up the running environment with the Dockerfile, please follow the steps 1-3 mentioned above.
